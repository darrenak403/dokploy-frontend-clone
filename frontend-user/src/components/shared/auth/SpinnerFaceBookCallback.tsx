...
const SpinnerFacebookCallback = () => {
  ...
  useEffect(() => {
    if (!apiUrl) {
      displayAlert("Không tìm thấy mã xác thực từ Facebook", "danger");
      setTimeout(() => router.replace("/signin?error=no_code"), 2000);
      return;
    }
    if (error) {
      console.error("Facebook callback error:", error);
      displayAlert("Đăng nhập Facebook thất bại. Vui lòng thử lại!", "danger");
      setTimeout(() => router.replace("/signin?error=facebook"), 2000);
    }
    if (data) {
      try {
        ...
        if (payload?.status === 200 && payload.data) {
          ...
          setTimeout(() => {
            router.replace("/signin?error=facebook");
          }, 2000);
        }
      } catch (e) {
        ...
        setTimeout(() => {
          router.replace("/signin?error=facebooksss");
        }, 2000);
      }
    }
...
export default SpinnerFacebookCallback;
